...
# Надто загальні слова, щоб бути видом. «Напій» об'єднує воду, колу,
# енергетик і комбучу — це не одна звичка, а чотири різні. Перевірено на
# живих даних: вісім різних напоїв, куплених по одному разу, злились в одну
# «стабільну звичку» з обличчям у вигляді комбучі.
GENERIC_HEADS = frozenset({
    "напій", "напої", "виріб", "вироби", "засіб", "суміш", "продукт",
    "набір", "асорті",
})

# Латиниця, що маскується під кирилицю. У чеках Сільпо трапляється:
# «Нaпій Живчик» написано з латинською a — і це давало окремий вид.
...
def kind_of(name: str) -> str:
    ...
    Два винятки. Латинські двійники нормалізуються, інакше «Нaпій» з
    латинською «a» стає окремим видом. А надто загальне головне слово
    («Напій», «Суміш») замінюється категорією — бо вода й енергетик не є
    однією звичкою.
    """
    cleaned = re.sub(r"[«»\"'()]", " ", (name or "").lower())
    words = [w.strip(".,:;-–—") for w in cleaned.split()]
    words = [w for w in words if len(w) >= 3 and w not in NOT_A_KIND]
    if not words:
        return "інше"

    # Нормалізуємо ЛИШЕ головне слово: до назв брендів чіпати латиницю не
    # можна, інакше «Spraga» перетворюється на «sрrаgа».
    head = ALIASES.get(words[0], words[0]).translate(LOOKALIKES)
    if head not in GENERIC_HEADS:
        return head

    # Загальне слово саме по собі нічого не означає, тож додаємо наступне —
    # зазвичай це марка або уточнення: «напій живчик», «суміш пластівців».
    #
    # Через категорію тут не йдемо навмисно: вона марковна за назвою і
    # помиляється («Живчик яблуко» потрапляв в «овочі та фрукти»). Марка ж
    # у назві стоїть завжди і бреше рідше.
    if len(words) > 1:
        return f"{head} {ALIASES.get(words[1], words[1])}"
    return head
...
def group(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for kind, members in buckets.items():
        # Обличчям виду стає найчастіша марка, а при рівності — НАЙСВІЖІША.
        # Раніше нічия розвʼязувалась ціною, і в гостя, який щоразу бере
        # інший напій, «звичкою» ставала найдорожча пляшка за півроку.
        #
        # Два стабільні сортування замість одного складеного ключа: дати —
        # рядки ISO, їх не можна інвертувати в кортежі разом із числом.
        members = list(members)
        members.sort(key=lambda r: max(r.get("days") or [""]), reverse=True)
        members.sort(key=lambda r: len(r.get("days") or []), reverse=True)
        days: set[str] = set()
