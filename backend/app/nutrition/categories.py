...
CATEGORY_MARKERS: dict[str, tuple[str, ...]] = {
    ...
    "alcohol": ("пиво", "вино", "віскі", "горілк", "лікер", "сидр", "коньяк", "шампан", "джин",
                "ром ", "текіл", "бренді"),
    # Нехарчове й напої без цукру раніше падали в «Інше» — через це вода
    # й тютюн опинялись у місячному циклі, хоча беруться щотижня.
    "water": ("вода мінеральна", "вода питна", "вода негазована", "вода сильногаз",
              "вода слабогаз", "моршинська", "миргородська", "боржомі", "borjomi"),
    "coffee_tea": ("кава", "чай ", "какао"),
    "tobacco": ("тютюнов", "сигарет", "стик", "terea", "heets", "delia", "iqos"),
    "household": ("папір туалетний", "серветк", "мило", "шампун", "гель", "порошок",
                  "засіб", "губка", "рушник паперов", "зубна", "дезодорант", "пральн"),
    "other": (),
}
...
# Категорії, у яких ми НІЧОГО не рекомендуємо і не збираємо в кошик.
#
# Це не мораль. Тютюн і алкоголь — персональний вибір, у якому підказка
# стороннього сервісу недоречна: марка стиків жорстко прив'язана до пристрою
# гостя, тож «схожий товар дешевше» тут просто неможливо купити. У витратах
# і аналітиці ці категорії лишаються — ми їх не ховаємо, лише не радимо.
NO_RECOMMENDATION: set[str] = {"tobacco", "alcohol"}
...
# Товари, які за назвою схожі на їжу, але їжею не є.
# «Корм для котів з індичкою» містить маркер «індич» і без цього фільтра
# потрапляв у білкові продукти — перевірено на живій видачі каталогу.
NON_FOOD_MARKERS: tuple[str, ...] = (
    "корм для", "корм ", "для котів", "для кішок", "для собак", "для тварин",
    "наповнювач", "лоток", "ласощі для",
)


# Товари, які ми НІКОЛИ не радимо проактивно.
#
# Купувати їх ніхто не забороняє — але радити дорослому дитячу кашу, щоб
# підтягнути «крупи», безглуздо. Так само приправи й готові страви: вони
# формально потрапляють у категорію, але основою раціону не є.
NEVER_RECOMMEND_MARKERS: tuple[str, ...] = (
    "дитяч", "для дітей", "малюк", "місяц", "milupa", "nutrilon", "hipp", "nestle baby",
    "корм", "для котів", "для кішок", "для собак", "для тварин",
    "майонез", "кетчуп", "соус", "приправ", "спеці", "оцет", "маргарин",
    "готов", "суші", "піца", "напівфабрикат",
)


def recommendable(name: str) -> bool:
    """Чи доречно пропонувати цей товар як пораду, а не просто як товар."""
    low = (name or "").lower()
    return not any(m in low for m in NEVER_RECOMMEND_MARKERS)


def categorize(name: str) -> str:
    """Категорія за маркером, що стоїть НАЙРАНІШЕ в назві.

    Назва українського товару починається з типу продукту: «Йогурт Ростишка
    банан» — це йогурт, а не фрукт. Перший збіг за порядком у словнику давав
    «овочі та фрукти», бо «банан» стоїть у списку раніше за «йогурт», і в
    рекомендації овочів потрапляв банановий йогурт.
    """
    low = (name or "").lower()
